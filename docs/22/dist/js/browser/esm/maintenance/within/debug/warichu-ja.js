// src/ts/maintenance/utils/error-class.ts
class ErrorClass {
...
var WarichuError2 = _errs.WarichuError;
var ALLOWED_PAIRS = {
  "(": ")",
  "[": "]",
  "{": "}",
...
// src/ts/maintenance/utils/get-segments.ts
...
// src/ts/maintenance/normalize.ts
...
function validateSyntax(syntax) {
  ...
  if (separator === open || separator === close) {
    throw new WarichuError2(`options.syntax.separator は enclosers と異なる文字である必要があります: ${separator}`);
  }
  const hasControl = (s) => /[\p{C}]/u.test(s);
  if (hasControl(open) || hasControl(close) || hasControl(separator)) {
    throw new WarichuError2("options.syntax に制御文字を含めることはできません。");
  }
}
...
class ExplicitSplitStrategy extends WarichuSplitStrategy {
  calc(graphemes) {
    ...
    if (graphemes.indexOf(separator, index + 1) !== -1) {
      throw new WarichuError2(`割注に分割記号「${separator}」が複数あります。一個まで有効です。`);
    }
    if (index === 0 || index === graphemes.length - 1) {
      throw new WarichuError2(`割注の片方の行が空です。指定分割位置は先頭・末尾以外の箇所であるべきです。: index:${index} (${index === 0 ? "先頭" : "末尾"})`);
    }
